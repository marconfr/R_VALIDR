...
compute_validation_data <- function() {
  ...
  for (var in what_to_grep) {
    ...
    for (j in levels(as.factor(VALID$CONC_LEVEL))) {
      ...
      VALID <- setDT(VALID)[, expected result := ifelse(test = TVL_RELATIVE > -nACC_LIMIT & TVH_RELATIVE < nACC_LIMIT, "expected result", "non-compliant result")]
    } 
    listVALID[[var]] <<- VALID
  }
}
...
#------------------------------------------------------------------------------#
# These functions search coordinate of LOQ 
#------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------#
# This function plot accuracy profile  
#------------------------------------------------------------------------------#
...
plot_validation <- function(data, string) {
  ...
  p <- ggplot(data, aes(x = CONC_LEVEL, y = ERROR_pc, color = as.factor(SERIE))) +
    ...
    geom_point() +
    geom_line(data = data %>% select(-SERIE, -expected result) %>% 
...
summary_validation_rel <- function(data, string) {
  ...
  t <- data %>%
    ...
    mutate(expected result = ifelse(TVL_RELATIVE > -nACC_LIMIT & TVH_RELATIVE < nACC_LIMIT, "expected result", "non-compliant result")) %>%
    kableExtra::kable(col.names = c(
      paste("Introduced concentrations ", sConcUnit),
      paste("Mean calculated concentrations ", sConcUnit),
      "Bias (%)",
...
plot_linearity <- function(data, string) {
  p <- ggplot(data, aes(x = CONC_LEVEL, y = data[, 4])) +
    ...
    geom_line(data = data %>% select(-SERIE, -expected result) %>% group_by(CONC_LEVEL) %>% summarize_all(~ mean(.x, na.rm = TRUE)), aes(x = CONC_LEVEL, y = TVL_ABS), color = "blue") +
    geom_line(data = data %>% select(-SERIE, -expected result) %>% group_by(CONC_LEVEL) %>% summarize_all(~ mean(.x, na.rm = TRUE)), aes(x = CONC_LEVEL, y = TVH_ABS), color = "blue") +
...
ggplotly(plot_validation(as.data.frame(listVALID[["LIN"]]), names(listVALID["LIN"])))%>% config(
    toImageButtonOptions = list(
      format = ""
    ))
...
Linearity profile is shown Figure \@ref(fig:lplin). You should check that the black line (linear model) should be superimposed on the red dashed identity line.
...
ggplotly(plot_linearity(as.data.frame(listVALID[["LIN"]]), names(listVALID["LIN"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
`r if(summary(linear_model_stat(listVALID[["LIN"]]))$coefficients[1,4]<0.05){ "The p-value for the intercept is < 0.05 which is in favor of a significant value (non-compliant result)"}else{ "The p-value for the intercept is > 0.05 which is in favor of a non-significant value (expected result)"}`

`r if(summary(linear_model_stat(listVALID[["LIN"]]))$coefficients[2,4]<0.05){ "The p-value for the slope is < 0.05 which is in favor of a significant value (expected result)"}else{ "The p-value for the slope is > 0.05 which is in favor of a non-significant value (non-compliant result)"}`
...
## Using a linear calibration curve with the highest level only and forced through 0

### Trueness and precision obtained

Trueness and precision are depicted on the table \@ref(tab:iplinmaxabs) and \@ref(tab:iplinmaxrel)
...
Accuracy profile is shown in Figure \@ref(fig:aplinmax). The 𝛽-tolerance interval ([─]{style="color:blue"}) should be entirely within the acceptance limits ([┄]{style="color:red"}). Coordinates of points of intersection between the tolerance interval and the acceptability limit were marked with a red star ([✴]{style="color:red"}) when present.
...
```{r aplinmax, echo=FALSE, fig.width=10, fig.height=4, fig.fullwidth=TRUE, fig.cap="Accuracy profiles  (red dashed line: acceptance limits, blue lines: 𝛽-tolerance intervals)."}
  ...
  ggplotly(plot_validation(as.data.frame(listVALID[["LIN_0MAX"]]), names(listVALID["LIN_0MAX"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
Linearity profile is shown in Figure \@ref(fig:lplinmax). You should check that the black line (linear model) should be superimposed on the red dashed identity line.
...
```{r lplinmax, echo=FALSE, fig.width=10, fig.height=4, fig.fullwidth=TRUE, fig.cap="Linearity profiles (red dashed line: identity line, black lines: linear regression lines, blue lines: 𝛽-tolerance intervals)."}
  ...
  ggplotly(plot_linearity(as.data.frame(listVALID[["LIN_0MAX"]]), names(listVALID["LIN_0MAX"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
`r if(summary(linear_model_stat(listVALID[["LIN_0MAX"]]))$coefficients[1,4]<0.05){ "The p-value for the intercept is < 0.05 which is in favor of a significant value (non-compliant result)"}else{ "The p-value for the intercept is > 0.05 which is in favor of a non-significant value (expected result)"}`

`r if(summary(linear_model_stat(listVALID[["LIN_0MAX"]]))$coefficients[2,4]<0.05){ "The p-value for the slope is < 0.05 which is in favor of a significant value (expected result)"}else{ "The p-value for the slope is > 0.05 which is in favor of a non-significant value (non-compliant result)"}`
...
Trueness and precision are depicted on the table \@ref(tab:iplin1yabs) and \@ref(tab:iplin1yrel)
...
Accuracy profile is shown in Figure \@ref(fig:aplin1y). The 𝛽-tolerance interval ([─]{style="color:blue"}) should be entirely within the acceptance limits ([┄]{style="color:red"}). Coordinates of points of intersection between the tolerance interval and the acceptability limit were marked with a red star ([✴]{style="color:red"}) when present.
...
ggplotly(plot_validation(as.data.frame(listVALID[["LIN_1Y"]]), names(listVALID["LIN_1Y"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
Linearity profile is shown in Figure \@ref(fig:lplin1y). You should check that the black line (linear model) should be superimposed on the red dashed identity line.
...
ggplotly(plot_linearity(as.data.frame(listVALID[["LIN_1Y"]]), names(listVALID["LIN_1Y"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
`r if(summary(linear_model_stat(listVALID[["LIN_1Y"]]))$coefficients[1,4]<0.05){ "The p-value for the intercept is < 0.05 which is in favor of a significant value (non-compliant result)"}else{ "The p-value for the intercept is > 0.05 which is in favor of a non-significant value (expected result)"}`

`r if(summary(linear_model_stat(listVALID[["LIN_1Y"]]))$coefficients[2,4]<0.05){ "The p-value for the slope is < 0.05 which is in favor of a significant value (expected result)"}else{ "The p-value for the slope is > 0.05 which is in favor of a non-significant value (non-compliant result)"}`
...
Trueness and precision are depicted on the table \@ref(tab:iplin1xabs) and \@ref(tab:iplin1xrel)
...
Accuracy profile is shown in Figure \@ref(fig:aplin1x). The 𝛽-tolerance interval ([─]{style="color:blue"}) should be entirely within the acceptance limits ([┄]{style="color:red"}). Coordinates of points of intersection between the tolerance interval and the acceptability limit were marked with a red star ([✴]{style="color:red"}) when present.
...
ggplotly(plot_validation(as.data.frame(listVALID[["LIN_1X"]]), names(listVALID["LIN_1X"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
Linearity profile is shown in Figure \@ref(fig:lplin1x). You should check that the black line (linear model) should be superimposed on the red dashed identity line.
...
ggplotly(plot_linearity(as.data.frame(listVALID[["LIN_1X"]]), names(listVALID["LIN_1X"])))%>% config(
    toImageButtonOptions = list(
      format = "svg"
    ))
...
`r if(summary(linear_model_stat(listVALID[["LIN_1X"]]))$coefficients[1,4]<0.05){ "The p-value for the intercept is < 0.05 which is in favor of a significant value (non-compliant result)"}else{ "The p-value for the intercept is > 0.05 which is in favor of a non-significant value (expected result)"}`

`r if(summary(linear_model_stat(listVALID[["LIN_1X"]]))$coefficients[2,4]<0.05){ "The p-value for the slope is < 0.05 which is in favor of a significant value (expected result)"}else{ "The p-value for the slope is > 0.05 which is in favor of a non-significant value (non-compliant result)"}`
